...
class SelfAttentive(object):
  ...
  def build_graph(self, n=60, d=100, u=128, d_a=350, r=30, reuse=False):
    with tf.variable_scope('SelfAttentive', reuse=reuse):
      ...
      # shape(W_s1) = d_a * 2u
      self.W_s1 = tf.get_variable('W_s1', shape=[self.d_a, 2*self.u],#[350，2*128]
          initializer=initializer)
      # shape(W_s2) = r * d_a
      self.W_s2 = tf.get_variable('W_s2', shape=[self.r, self.d_a],#[30,350]
          initializer=initializer)

      # BiRNN
      self.batch_size = batch_size = tf.shape(self.input_pl)[0]

      cell_fw = ON_LSTM(u,16)
      cell_bw = ON_LSTM(u,16)
